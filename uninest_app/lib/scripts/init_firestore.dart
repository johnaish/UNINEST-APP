...
Future<void> main() async {
  ...
  try {
    ...
    print('  ✓ Created admin user (admin-user-123)');
    ...
    print('  ✓ Created landlord user (landlord-user-456)');
    ...
    print('  ✓ Created student user (student-user-789)');

    // Create properties collection with sample property
    print('\nCreating properties collection...');
    final propRef = await db.collection('properties').add({
    ...
    print('  ✓ Created sample property (${propRef.id})');

    // Create bookings collection (empty, ready for use)
    print('\nCreating bookings collection...');
    await db.collection('bookings').doc('_init').set({'_init': true});
    await db.collection('bookings').doc('_init').delete();
    print('  ✓ Created bookings collection');

    // Create messages collection (empty, ready for use)
    print('\nCreating messages collection...');
    await db.collection('messages').doc('_init').set({'_init': true});
    await db.collection('messages').doc('_init').delete();
    print('  ✓ Created messages collection');

    // Create conversations collection (empty, ready for use)
    print('\nCreating conversations collection...');
    await db.collection('conversations').doc('_init').set({'_init': true});
    await db.collection('conversations').doc('_init').delete();
    print('  ✓ Created conversations collection');

    print('\n✅ Firestore initialization complete!');
    print('\nCollections created:');
    print('  • users (3 sample users: admin, landlord, student)');
    print('  • properties (1 verified sample property)');
    print('  • bookings (empty, ready for use)');
    print('  • messages (empty, ready for use)');
    print('  • conversations (empty, ready for use)');
    print('\nYou can now register/login in the app and start using Firestore!');
  } catch (e) {
